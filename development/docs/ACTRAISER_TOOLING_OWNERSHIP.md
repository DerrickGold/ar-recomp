...
The Quintet bit-packed LZSS implementation and its `quintet-lzss` diagnostic
command are owned by `installer/`. `tools/quintet_lzss.py` remains deliberately
as an independent implementation used as an oracle by ActRaiser's
collision/content research rather than production tooling.
