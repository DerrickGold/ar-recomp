...
int main(void) {
  ...
  assert(!ActRaiserRegional_PoseSnapshot());
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_AitosPoses,1)==kActRaiserRegionalEdit_Applied);
  assert(!ActRaiserRegional_PoseSnapshot());
  uint16_t pose_time;
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_Sequences,1)==kActRaiserRegionalEdit_Applied);
  bool sequence;
  assert(!ActRaiserRegional_BeginSongSequence(2,&sequence) && !ActRaiserRegional_BeginSongSequence(0,NULL));
  assert(ActRaiserRegional_BeginSongSequence(0,&sequence) && sequence);
  assert(ActRaiserRegional_CopyRulesView(&view) && view.effective.sequences.source[0]==1 && !view.effective.sequences.source[1]);
  assert(ActRaiserRegional_BeginSongSequence(1,&sequence) && sequence);
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_ActorArt,1)==kActRaiserRegionalEdit_Applied);
  bool actor_art;
  assert(!ActRaiserRegional_ActorArtwork(7,true,&actor_art) && !ActRaiserRegional_ActorArtwork(0,true,NULL));
  assert(ActRaiserRegional_ActorArtwork(0,false,&actor_art) && actor_art);
  assert(ActRaiserRegional_CopyRulesView(&view) && !view.effective.actor_artwork.source[0]);
  assert(ActRaiserRegional_ActorArtwork(0,true,&actor_art) && actor_art);
  assert(ActRaiserRegional_CopyRulesView(&view) && view.effective.actor_artwork.source[0]==1 && !view.effective.actor_artwork.source[1]);
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_ActorArt,2)==kActRaiserRegionalEdit_Applied);
  assert(ActRaiserRegional_ActorArtwork(0,true,&actor_art) && !actor_art);
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&pose_time) && ActRaiserRegional_PoseSnapshot()==3);
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_AitosPoses,2)==kActRaiserRegionalEdit_Applied);
  assert(ActRaiserRegional_PoseSnapshot()==3);
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&pose_time) && !ActRaiserRegional_PoseSnapshot());
  assert(ActRaiserRegional_CopyRulesView(&view));
  ...
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_DeathHeimArt,1)==kActRaiserRegionalEdit_Applied);
  assert(!ActRaiserRegional_ArtworkSnapshot());
  assert(!ActRaiserRegional_BeginActionRoom(3,0x300,NULL) && !ActRaiserRegional_ArtworkSnapshot());
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&motion_time) && ActRaiserRegional_ArtworkSnapshot()==1);
  assert(ActRaiserRegional_CopyRulesView(&view) && view.effective.artwork.source[0]==1 && !view.artwork_available);
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_DeathHeimArt,2)==kActRaiserRegionalEdit_Applied);
  assert(ActRaiserRegional_ArtworkSnapshot()==1);
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&motion_time) && !ActRaiserRegional_ArtworkSnapshot());
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_DeathHeimArt,0)==kActRaiserRegionalEdit_Applied);
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&motion_time) && !ActRaiserRegional_ArtworkSnapshot());
  assert(ActRaiserRegional_CopyRulesView(&view));
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_ActionItemArt,2)==kActRaiserRegionalEdit_Applied);
  assert(!ActRaiserRegional_ArtworkSnapshot());
  assert(!ActRaiserRegional_BeginActionRoom(3,0x300,NULL) && !ActRaiserRegional_ArtworkSnapshot());
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&motion_time) && ActRaiserRegional_ArtworkSnapshot()==2);
  assert(ActRaiserRegional_CopyRulesView(&view) && view.effective.artwork.source[1]==2);
  assert(ActRaiserRegional_RequestRules(&view,kActRaiserRegionalSetting_ActionItemArt,0)==kActRaiserRegionalEdit_Applied);
  assert(ActRaiserRegional_ArtworkSnapshot()==2);
  assert(ActRaiserRegional_BeginActionRoom(3,0x300,&motion_time) && !ActRaiserRegional_ArtworkSnapshot());
  assert(ActRaiserRegional_CopyRulesView(&view));
  uint8_t music_profile=255;
  for(unsigned rule=2;rule<5;++rule) {
    const unsigned group=kActRaiserRegionalSetting_FollowerArt+rule-2;
    uint8_t town_art=255;
    assert(ActRaiserRegional_RequestRules(&view,group,1)==kActRaiserRegionalEdit_Applied);
    assert(ActRaiserRegional_BeginActionRoom(3,0x300,&motion_time));
    assert(ActRaiserRegional_CopyRulesView(&view) && !view.effective.artwork.source[rule]);
    assert(!ActRaiserRegional_BeginTownArtwork(0x101,&town_art) && town_art==255);
    assert(!ActRaiserRegional_BeginTownArtwork(0x700,&town_art) && town_art==255);
    assert(!ActRaiserRegional_BeginTownArtwork(0x100,NULL));
    assert(ActRaiserRegional_BeginTownArtwork(0x100,&town_art) && town_art==(1u<<rule));
    assert(ActRaiserRegional_TownArtworkSnapshot(0x100)==town_art && !ActRaiserRegional_TownArtworkSnapshot(0x200));
    assert(ActRaiserRegional_CopyRulesView(&view));
  ...
  cpu=(CpuState){.PB=2,.DB=2,.m_flag=1,.S=0x1ee0};ram[0x336]=1;
  uint8_t title_art=255;
  assert(!ActRaiserRegional_BeginTitleArtwork(NULL));
  ...
  assert(!ActRaiserRegional_TitleArtworkSnapshot());
  assert(ActRaiser_RegionalTitle(&cpu)==RECOMP_RETURN_NORMAL);
  assert(ActRaiserRegional_CopyRulesView(&view) && !view.new_game && !view.requested.spell_inventory);
  assert(!view.requested.artwork.source[kArRegionalArtwork_TitleBackground]); /* Continue discards title draft. */
  ...
  assert(ActRaiserRegional_CopyRulesView(&view) && view.new_game && view.effective.artwork.source[kArRegionalArtwork_TitleBackground]==1);
