...
static void CheckSessionState(void) {
  ...
  CHECK(ArRegionalCosts_Init(&defaults, kArRegionalCost_US));
  ...
  CHECK(ArRegionalSession_RequestCosts(&session, 1, kArRegionalCostGroup_Miracles, kArRegionalCost_Japan));
  CHECK(session.revision == 2 && session.effective.source[kArRegionalCost_Rain] == kArRegionalCost_US);
  CHECK(!ArRegionalSession_RequestCosts(&session, 1, kArRegionalCostGroup_Miracles, kArRegionalCost_US));
  ...
  CHECK(ArRegionalSession_RequestCosts(&session, 3, kArRegionalCostGroup_Miracles, kArRegionalCost_US));
  CHECK(quote.price[kArRegionalCost_Rain] == 16); /* An accepted quote stays frozen. */
  CHECK(session.effective.source[kArRegionalCost_Rain] == kArRegionalCost_Japan);
  CHECK(ArRegionalSession_BeginCosts(&session, kArRegionalCostGroup_Miracles, &quote));
  CHECK(quote.price[kArRegionalCost_Rain] == 20 && session.revision == 5);
  session.revision = UINT32_MAX;
  before = session;
  CHECK(!ArRegionalSession_RequestCosts(&session, UINT32_MAX, kArRegionalCostGroup_Scrolls, kArRegionalCost_Japan));
  CHECK(EqualSession(&before, &session));
  CHECK(ArRegionalSession_RequestCosts(&session, UINT32_MAX, kArRegionalCostGroup_Scrolls, kArRegionalCost_US));
  CHECK(ArRegionalSession_BeginCosts(&session, kArRegionalCostGroup_Scrolls, &quote));
  CHECK(EqualSession(&before, &session));
  session.requested.source[kArRegionalCost_Light] = kArRegionalCost_Japan;
  before = session;
...
static void CheckPersistence(SaveFileFormat format, const char *path) {
  ...
  CHECK(ArRegionalSession_RequestCosts(&session, session.revision, kArRegionalCostGroup_Miracles, kArRegionalCost_Japan));
  CHECK(!MAKE_DIR(native_tmp));
  ...
  CHECK(ArRegionalSession_RequestCosts(&session, session.revision, kArRegionalCostGroup_Miracles, kArRegionalCost_US));
  ...
  CHECK(EqualSession(&loaded, &session));
  CHECK(ArRegionalSession_RequestCosts(&session, session.revision, kArRegionalCostGroup_Scrolls, kArRegionalCost_Japan));
  ...
  memcpy(future, payload, payload_size); future[8] = 2;
...
static void CheckFeatureCodec(void) {
  ...
  for (unsigned mutation = 0; mutation < 9; ++mutation) {
    ...
    switch (mutation) {
      ...
      case 1: mutated[8] = 2; expected = kSaveCheckpoint_Unsupported; break;
  ...
  size_t offsets[kArRegionalCostRule_Count], offset = 36;
  ...
  for (unsigned i = kArRegionalCostRule_Count; i-- > 0;) {
