...
static void Run(const PlacementRoot *root,const ArRegionalPlacementPolicy *policy,bool mode,unsigned d,unsigned terrain) {
  ...
  unsigned x=0xae0,objects=0,gate=0;size_t after=0;
...
int main(void) {
  ...
  for(unsigned e=0;e<3;++e)for(unsigned p=0;p<3;++p)for(unsigned mode=0;mode<2;++mode)
    for(unsigned d=0;d<3;++d)for(unsigned r=0;r<49;++r) {
      const ArRegionalPlacementPolicy policy={e,p};Run(&kRoots[r],&policy,mode,d,(e+p+d)%3);
    }
  ...
  puts("Native placement adapters: complete pool footprint, waves, captures, cursor/return contracts passed");
  return 0;
}
