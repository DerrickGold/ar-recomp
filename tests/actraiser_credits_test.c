...
RecompReturn bank_02_AEEB_M1X0(CpuState *cpu) {
  ++uploads;
  CHECK(!ActRaiser_CreditsUploadEntry(cpu));
...
static void Upload(bool lower_rows) {
  ...
  CHECK(ActRaiser_CreditsUpload(&cpu) == upload_result);
...
static void TestMissingOrInvalidEvidence(void) {
  ...
  CpuState cpu = Cpu();
  cpu.DB = 0x7f; CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  cpu = Cpu(); cpu.D = 0x100; CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  cpu = Cpu(); cpu.x_flag = 1; CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  cpu = Cpu(); cpu.m_flag = 0; CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  cpu = Cpu(); cpu.PB = 1; CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  cpu = Cpu(); cpu.emulation = 1; CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  CHECK(!ActRaiser_CreditsUploadEntry(NULL));
...
static void TestSceneResetAndNativeReturn(void) {
  ...
  CpuState cpu = Cpu();
  CHECK(!ActRaiser_CreditsUploadEntry(&cpu));
  ActRaiserCredits_ObserveScene(0, 1);
  CHECK(ActRaiserCredits_PresentedPage() == kActRaiserCreditsNoPage);
}
