...
import (
	...
	"github.com/DerrickGold/ar-recomp/installer/internal/audio"
)
...
func audioPreviewTracks() []audio.Track {
	tracks := make([]audio.Track, 0, len(assetTracks))
...
func (app *application) startAudioPreviews(response http.ResponseWriter,
	...
	if request.URL.Query().Get("force") == "1" {
		fingerprint, fingerprintErr := audio.ROMFingerprint(romPath)
		if fingerprintErr != nil {
			writeJSONError(response, http.StatusBadRequest, fingerprintErr.Error())
			return
		}
		stale := audio.PreviewCacheDirectory(cacheRoot, fingerprint, audioPreviewDuration)
	...
	go func() {
		paths, fingerprint, renderErr := audio.RenderActRaiserPreviews(
			app.ctx, romPath, cacheRoot, audioPreviewTracks(), audioPreviewDuration,
			func(progress audio.Progress) {
				app.previewMu.Lock()
