...
(() => {
  ...
  function errorMessage(error){
    ui.set(status,"builder.media.failed");
    window.workshopFeedback?.show(status,error,{operation:"Install regional media",retry:load});
  }
  ...
  async function inventory(){
    const data=await read(await fetch("regional-media"));
  ...
  function load(){return run(async()=>{await inventory();ui.set(status,"builder.media.ready");});}
  async function install(file,replace){
    const body=new FormData();body.append("donor",file);body.append("replace",String(replace));
    const response=await fetch("regional-media",{method:"POST",body});
