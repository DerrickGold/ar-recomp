...
let confirms=0,allow=false,feedback=0;const bindings=[];
global.window={confirm:()=>{confirms++;return allow;},workshopI18n:{set:(el,key,args)=>{el.key=key;el.args=args;bindings.push(key);},text:key=>key},workshopFeedback:{clear(){},show(){feedback++;}}};
