...
//
// This lives in cmd/actraiser-builder rather than in internal/builder because it is
...
//	  tools/actraiser-builder  KEEP: run-build and runtime languages use it
//	  tools/snesbuild          BUILD ONLY
...
// NOT "tools": that directory holds actraiser-builder itself, and every
// run-build script gates on it before doing anything else --
//
...
// own marker. The generic driver is removed individually with the other rebuild
// inputs.
//
...
// snesbuild is needed only while rebuild inputs remain. actraiser-builder is
// intentionally absent: the game invokes it to prepare .arlang archives even
// after the user keeps just the playable game.
...
// bundle root).
func detectInstallState(root, outputDir string) builder.InstallState {
	state := builder.InstallState{}
	...
	if state.CanLaunch && isSlimmableBundle(root) {
		...
		if !state.CanSlim {
			for _, relative := range buildOnlyFiles {
...
func runtimeBuildInputAvailable(root string) bool {
	...
	archive, err := nativeRuntimeArchivePath(runtimeDir)
	if err != nil {
		return false
	}
...
func nativeRuntimeArchivePath(runtimeDir string) (string, error) {
	architecture := map[string]string{"amd64": "x86_64", "arm64": "aarch64"}[runtime.GOARCH]
	operatingSystem := map[string]string{
		"darwin": "macos", "linux": "linux-gnu", "windows": "windows-gnu",
	}[runtime.GOOS]
	if architecture == "" || operatingSystem == "" {
		return "", fmt.Errorf("no vended runtime for %s/%s", runtime.GOOS, runtime.GOARCH)
	}
	archive := "libsnesrecomp_runtime.a"
	if runtime.GOOS == "windows" {
		archive = "snesrecomp_runtime.lib"
	}
	return filepath.Join(runtimeDir, "lib", architecture+"-"+operatingSystem, archive), nil
}
...
//
// This guard is mandatory: `actraiser-builder gui --root .` may target a checkout,
...
//
//   - the bundled Builder lives at tools/actraiser-builder[.exe]; a checkout builds it
...
// (builder.storeROM); game.sfc is the CLI's default name (project.DefaultPaths)
