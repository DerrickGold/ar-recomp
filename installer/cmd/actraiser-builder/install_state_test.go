...
func bundleFixture(t *testing.T) (root, utils string) {
	...
	if err := os.WriteFile(filepath.Join(utils, "tools", "actraiser-builder"),
		[]byte("binary"), 0o755); err != nil {
		t.Fatal(err)
	}
	write(filepath.Join(utils, "tools", "snesbuild"), "generic build driver")
	...
	runtimeArchive, err := nativeRuntimeArchivePath(filepath.Join(utils, "snesrecomp-go", "runtime"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSlimRemovesOnlyBuildFiles(t *testing.T) {
	...
	mustSurvive := []string{
		// actraiser-builder itself. Every run-build script gates on this before doing
		// anything ([ -x "$UTILS/tools/actraiser-builder" ] || fail "This package looks
		...
		filepath.Join(utils, "tools", "actraiser-builder"),
...
func TestCleanupDoesNotFollowSymlinksOutOfTheBundle(t *testing.T) {
	...
	// utils/tools (and the tools/actraiser-builder bundle marker) intact -- otherwise the
...
func TestCleanupRefusesInASourceCheckout(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(repo, "tools", "actraiser-builder"),
...
func TestCleanupLeavesRunBuildAbleToStart(t *testing.T) {
	guards := map[string]string{
		"run-build.sh":      "tools/actraiser-builder",
		"run-build.command": "tools/actraiser-builder",
		"run-build.bat":     `tools\actraiser-builder.exe`,
	}
	checked := 0
	...
	// The POSIX guard's own test: -x on utils/tools/actraiser-builder.
	info, err := os.Stat(filepath.Join(utils, "tools", "actraiser-builder"))
	if err != nil {
		t.Fatalf("utils/tools/actraiser-builder is gone after the cleanup, so every "+
			"run-build script aborts with \"This package looks incomplete\" and "+
			"launcher mode cannot be reached: %v", err)
	}
	if runtime.GOOS != "windows" && info.Mode().Perm()&0o111 == 0 {
		t.Fatal("utils/tools/actraiser-builder survived but is not executable, so the " +
