...
Resolved case law lives in [docs/bug-ledger.md](docs/bug-ledger.md); current
feature acceptance lives in [docs/progress.md](docs/progress.md). Keep only
reproducible, unresolved work here, and move it to the ledger once verified.

### Ending and credits presenter — fix landed, verification pending

Mode `$18=08` previously missed the hand-rolled long-jump target `$02:AA9C`,
skipping the credits presenter after the ending montage. The configured
`$02:AA9C` entry and its `$00:8059` main-loop return target close both long-jump
sites; verify the credits sequence, Start-button exit, and `$70:1FF0-$1FF2`
completion marker in a current natural run. Revisit the one-shot `$01:ACD9`
width observation only if the ending prelude shows visible corruption.
---
...
quintet_lzss.py        independent Python oracle for the shared C decoder (`src/quintet_lzss.c`) and whole-body `$02:C5C9` HLE. The format is bit-packed, with the ring pre-filled by $20, cursor at $EF, and match len = nibble+2. `--verify` checks two known blobs byte-for-byte against saves/dump_wram.bin; `--selfcheck` cross-checks the bit reader against a literal $C66C transcription. CAUTION: a naive byte-aligned LZSS decoder appears to work on this data (it consumes the stream and emits plausible bytes) but is wrong — always validate a decoder against a known blob before trusting its output
...
AR_SIM_STRUCTURE_DEBUG=1 [sim-structure] one transition-only line for every enhanced house/bridge/windmill/factory: cell, live metatile, common visual state/phase, record/action and step slot when present, plus explicit UNMATCHED entries. First tool for a missing or stuck building animation. `AR_WINDMILL_DEBUG=1` remains an alias (ledger 61/83, SEAMS town 7)
