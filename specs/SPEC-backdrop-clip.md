...
| `bounded_world_margins` active by default? | **Yes, in the configuration where the symptom exists.** `ws_bgrefresh` defval = 1 (`src/settings.c:1738-1741`); boot derives `kDisplayMode_WideFull` via `InferDisplayMode`. But the whole symptom needs **three** non-default user selections: `extended_aspect != 4:3` (default 4:3, `src/settings.c:843`, `config.ini:19` → `g_ws_extra == 0` → no margins at all), `diorama_mode = On` (default 0, `src/settings.c:1234`), and for the skybox contributor `diorama_skybox != Off` (default Off, `src/settings.c:1282`). This is user-reachable menu configuration, not exotic. |
...
| — | Shoebox, HUD/`PresentHudOverlayComposited`, margin refresher, tile streamer, `PpuWriteOverlayRenderLine` writeback | — | **Verified non-contributors.** Do not touch. |
...
**File:** `snesrecomp-go/runtime/src/snes/ppu.c`, functions `PpuMergePaddedBackground` (:987) and its only caller `PpuDrawBackground_4bpp_policy` (:1014-1042).
...
**Rejected**
- **Comment's option (a), a per-layer numeric ceiling in `PpuLayerExtra`** — *inert* for the dominant case: `ppu.c:500-503` returns 0 for any layer with a clamp/mirror/repeat bit **before** the numeric `extra` argument is consulted, and BG2 always has one of those bits when `bg2_width < 512`. For wide BG2 it is *harmful*: it widens the tilemap **fetch**, and the host margin refresher sizes its strips from `extraLeftCur/extraRightCur` (`src/actraiser_widescreen_bg.c:664-667,754-757`), so the new columns would read unrefreshed/wrapped tilemap — the exact frozen-tile garbage the mirror/repeat machinery exists to avoid. Also on a ~3.5-7k-calls/frame hot path.
