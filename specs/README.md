...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **Proposed** | BH1. Unified action-background world provider + per-layer/per-band scene plan. Offline WRAM/VRAM oracle tools already exist (BG2 exact on 4 Fillmore snapshots); no production C decoder/provider is implemented. The existing PPU remains responsible for live VRAM/CGRAM, HDMA, priority, windows and color math. BH1-BH8 stage evidence, decoder, policy parity, margin-only adoption, full-frame parity, decorative/Death Heim coverage, default-on soak and cleanup. |
