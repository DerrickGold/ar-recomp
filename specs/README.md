...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress** | BH1/BH2 differential validation. The bounded pure C decoder, ROM-free suites, default-off observer, PPU-complete snapshot census, and isolated ordinary-entry runner are implemented. Region `$01-$06` act entries add 19,072,823 zero-mismatch runtime comparisons, 43,999 offline checks, and 12 visually inspected flat framebuffers; special rooms, handoffs, priority planes, positive controls, Northwall boss, and Death Heim still gate provider integration. The existing PPU remains responsible for live VRAM/CGRAM, HDMA, priority, windows and color math. |
