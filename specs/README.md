...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress** | BH2 foundation. The bounded pure C `ActionBgWorld` decoder and ROM-free suite are implemented and shipped inert (no production caller); the full BH1 census and runtime comparator still gate provider integration. The existing PPU remains responsible for live VRAM/CGRAM, HDMA, priority, windows and color math. BH1-BH8 stage evidence, policy parity, margin-only adoption, full-frame parity, decorative/Death Heim coverage, default-on soak and cleanup. |
