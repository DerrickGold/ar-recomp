...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress — BH8 cleanup underway** | BH7's bounded decoder/provider and exact `FrameSlot`/diorama handoff are default-on with `AR_ACTION_BG_HLE=0` authentic-center fallback; the PPU still owns VRAM/CGRAM, HDMA, priority, windows, transparency, mosaic and color math. BH8 has removed the action-world ring-repair transaction, builder trampolines, 128 KiB snapshot, and vertical band repair; `AR_WS_BGREFRESH` is now a hidden load-only alias. Unbound planned world layers clamp safely to the authentic viewport, while Wide Raw stays raw. Pre/post cleanup 4:3, Wide Full, and diorama-32 matrices accept 612/612 artifacts: framebuffers/state/PPU registers/authentic rings are exact, and seven full-VRAM files contain only the intended offscreen tilemap-write removal. Native streamers/ring/oracle and the unrelated Sky Palace repair remain. Final policy/setter census and release gates remain. |
