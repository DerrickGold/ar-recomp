...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress** | BH2 differential validation. The bounded pure C `ActionBgWorld` decoder, ROM-free suites, and default-off read-only runtime comparator are implemented; Fillmore act 2 has 6,729,804 zero-mismatch tile comparisons and a byte-identical state A/B. The full BH1 matrix still gates provider integration. The existing PPU remains responsible for live VRAM/CGRAM, HDMA, priority, windows and color math. BH1-BH8 stage evidence, policy parity, margin-only adoption, full-frame parity, decorative/Death Heim coverage, default-on soak and cleanup. |
