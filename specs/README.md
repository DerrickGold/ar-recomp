...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress** | BH1/BH2 differential validation and BH3 policy parity are implemented. The bounded decoder, observer, PPU-complete census, and entry runner cover 19,072,823 ordinary-entry comparisons, 1,032,404 direct Death Heim samples, and 6,646,861 in-world fetches across a native route through `0706`, all without mismatch. The pure `ActionBgPlan` now owns all 49 action-map clamp/mirror/repeat classifications; old/new builds are byte-identical across the entry, wide-policy, and vertical/diorama matrices. No HLE tile source is bound yet. The `0707`/final/ending tail, priority planes, positive controls, and a valid Northwall boss capture remain. Direct `0608` is rejected because matching tile words coexist with corrupt CHR. The existing PPU still owns VRAM/CGRAM, HDMA, priority, windows and color math. |
