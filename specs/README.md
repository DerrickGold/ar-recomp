...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress** | BH1/BH2 differential validation. The bounded decoder, ROM-free suites, default-off observer, PPU-complete census, and isolated entry runner are implemented. Ordinary entries add 19,072,823 zero-mismatch comparisons; direct Death Heim rooms classify all raw maps and add 1,032,404. A native Death Heim route adds 6,646,861 matching in-world fetches across eight source replacements through `0706`; its only finite exits identify the expected narrow-BG2 repeat/clamp seam. The `0707`/final/ending tail, priority planes, positive controls, and a valid Northwall boss capture remain. Direct `0608` is rejected because matching tile words coexist with corrupt CHR, confirming that tile parity is not pixel parity. The existing PPU still owns VRAM/CGRAM, HDMA, priority, windows and color math. |
