...
| [ar-recomp-sim-rendering-plan.md](ar-recomp-sim-rendering-plan.md) | **Partly implemented** | Simulation-town 3D rendering. Phases 0–4 and the core Phase 5 presentation work have landed; current metadata checkpoints require zero accounting errors. Fillmore, Bloodpool, Kasandora, and Marahna are accepted end-to-end, while Aitos and Northwall still need completion baselines and visual tuning. Where the plan says "present thread," read "presentation path"—the FrameSlot/upload contract remains, but the thread was removed. |
