...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **Proposed** | BH1. Native background decode for action stages. The decode is fully mapped and the oracle already passes (BG2 exact on 4 snapshots); nothing implemented. Key finding: the level map is **already decompressed in WRAM `$7E`**, so there is no ROM stream to reverse. |
