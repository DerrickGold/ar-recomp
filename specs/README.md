...
| [SPEC-bg-hle.md](SPEC-bg-hle.md) | **In progress — BH6 complete** | The bounded decoder, observer, 49-map `ActionBgPlan`, and default-off BH5 provider are implemented. After exact live-ring/scroll/bounds preflight, eligible world layers own authentic and margin tile words while the PPU retains VRAM/CGRAM, HDMA, priority, windows, transparency, mosaic and color math. The 12-entry provider matrix binds 19,522/19,522 eligible layer-frames and all 204 compared artifacts match native; worst-span cost is 0.067 ms/frame. BH6 now latches the exact per-layer/per-band plan through `FrameSlot`, removes `DioramaBg2MarginSource`, and passes focused Bloodpool/Aitos/Northwall plus `0701-$0708` gates. Direct `0608` remains rejected because matching words coexist with corrupt CHR. BH7 lifecycle soak/default promotion and BH8 behavior-neutral legacy cleanup remain. |
