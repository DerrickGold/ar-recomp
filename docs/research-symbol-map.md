...
| `$03:A4B8` | `StructVisual_ArmStepProgram` | Mapped | Resolves class/variant through ROM `$03:D4D2` and arms the record's 8-byte step-machine slot at `$7F:77E7+rec*8`. | [SEAMS town §7](SEAMS.md#7-structure-records-the-128-structure-cap-and-miracle-damage-bank-03-mapped-2026-07-17) |
...
| `$03:9C43` | `SimScene_CopyMetatile` | Verified | Copies one 2×2 metatile (four tilemap words, attribute bit 9 cleared) from `$7E:3100 + id*8` into the quadrant-paged live town tilemap at the cell in `$7CAF/$7CB1`. | [SEAMS town §7](SEAMS.md#7-structure-records-the-128-structure-cap-and-miracle-damage-bank-03-mapped-2026-07-17) |
| `$03:9FCD/$03:9FE4` | `SimScene_WriteStructureMark` | Verified | One-cell and 2×2-cell structure-mark writers used by `$9CFB`; `$9710` maps the staged cell to `$7F:2000 + town/quadrant index`. | [SEAMS town §7](SEAMS.md#7-structure-records-the-128-structure-cap-and-miracle-damage-bank-03-mapped-2026-07-17) |
| `$03:A591` | `SimScene_ExecuteDrawList` | Verified | Interprets a ROM draw list as count + `{dx,dy,metatile}` triples and calls `$9C43` for each; `$9D4D` reaches it through the initial `$A4F7` rebuild-program step. | [SEAMS town §7](SEAMS.md#7-structure-records-the-128-structure-cap-and-miracle-damage-bank-03-mapped-2026-07-17) |
