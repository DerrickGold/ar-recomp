...
| `$02:C5C9` / `$C639` / `$C66C` | `Lzss_Decompress` / `Lzss_ReadByte` / `Lzss_ReadLength` | Verified | Quintet LZSS. 256-byte ring at `$7E:2000` pre-filled with `$20`, write cursor `$AF` = `$EF`, control mask `$AE` = `$80`. **Bit-packed**: `$C639` reads 8 bits and `$C66C` a 4-bit length (len = n+2) by shifting `$B7/$B8` by the current bit phase. Ported byte-exact in `tools/quintet_lzss.py`, validated against `$7E:4000`/`$5000`/`$8000` in a live dump. | [SEAMS content seams](SEAMS.md#content--randomizer-seams-mapped-2026-08-02) |
