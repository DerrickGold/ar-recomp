...
| `$00:8C98` | `ActionObject_RebuildOam` | Verified | Clears OAM shadow, emits the fixed HUD, scans action objects, applies visibility/activation state, and invokes the component emitter. The native pause/freeze path skips this routine even though emulated vblanks continue; a completed invocation reaches its common epilogue only after the HUD and every admitted object composition have returned. This makes completed calls—not vblank count—the verified host presentation-tick boundary. | [rendering §9](rendering-engine.md#9-oam--sprite-pipeline-action), [action presentation clock](rendering-engine.md#9a-host-action-lighting-and-particles) |
