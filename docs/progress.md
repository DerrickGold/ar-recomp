...
| Action widescreen BG/sprites | 🟡 | All ordinary stages and Death Heim are fully playable and visually validated: wide streaming, fast vertical rows, sprites, activation, narrow-BG2 mirror/repeat policies, HDMA/parallax scenes, bosses, and post-final-boss transitions behave correctly. 2026-08-09 starts `SPEC-bg-hle.md`: the pure bounded `ActionBgWorld` WRAM decoder, atomic cache/publication contract, and ROM-free maximum-capacity suite are implemented but intentionally have no render caller yet. The complete action-map census and runtime differential comparator gate any provider integration. Remaining on the legacy path: general camera/world-edge clamp for full presentation coverage. |
