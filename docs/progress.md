...
| Action widescreen BG/sprites | 🟡 | All ordinary stages and Death Heim are fully playable and visually validated on the legacy path: wide streaming, fast vertical rows, sprites, activation, narrow-BG2 mirror/repeat policies, HDMA/parallax scenes, bosses, and post-final-boss transitions behave correctly. 2026-08-09 `SPEC-bg-hle.md`: the pure bounded `ActionBgWorld`, ROM-free suites, default-off observer, PPU-complete census, and isolated ordinary-entry matrix are implemented with no render binding. Region `$01-$06` act entries contribute 19,072,823 zero-mismatch runtime comparisons, 43,999 offline checks, and 12 distinct visually inspected flat frames. Special rooms/handoffs, priority planes, positive controls, Northwall boss, and Death Heim still gate provider integration. Remaining on the legacy path: general camera/world-edge clamp for full presentation coverage. |
