...
| Magic casting | 🟡 | WORKS as of 2026-07-07 (was dead — blocked by our own knockback cheat, `DEBUG.md` #18). 2026-08-02: all four spells' controller/slot lifecycles, animation banks, timing, transforms, and composition geometry are statically mapped in `effects-hook-investigation.md`. 2026-08-03: Magical Fire is the first host-polish slice: four exact per-slot emitters, explicit resettable actor/phase/pulse lifecycle, generic captured geometry, bounded renderer-independent effect batching, portable additive lighting/deterministic particles, independent settings, and exact priority-plane flat+diorama projection are implemented with ROM-free tests. Its new visuals need live acceptance; Stardust/Aura/Light remain mapped and need implementation plus the focused sweep (`AR_ALL_MAGIC`/`AR_INF_MP`). |
