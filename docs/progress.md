...
| Action widescreen BG/sprites | 🟡 | All ordinary stages and Death Heim are fully playable and visually validated: wide streaming, fast vertical rows, sprites, activation, narrow-BG2 mirror/repeat policies, HDMA/parallax scenes, bosses, and post-final-boss transitions behave correctly. 2026-08-09 starts `SPEC-bg-hle.md`: the pure bounded `ActionBgWorld` WRAM decoder, atomic cache/publication contract, ROM-free maximum-capacity suite, and default-off read-only runtime comparator are implemented but intentionally have no render binding. Fillmore act 2 compares 6,729,804 authentic tile fetches with zero mismatch; comparator-off/on final WRAM, SRAM, dispatch log, and state are byte-identical. Full snapshots now pin PPU/CHR/BGSC provenance and `tools/bg_hle_census.py` machine-checks source residency, mutation variants, eligibility, and ring parity; the retained corpus exposes only six action captures across `$01/$02-$03`, so the region/Death Heim matrix remains open. The complete action-map replay matrix still gates provider integration. Remaining on the legacy path: general camera/world-edge clamp for full presentation coverage. |
