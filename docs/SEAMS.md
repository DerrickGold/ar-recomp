...
- The action-room command-7/6/5/4 CPU handoff is live without replacing this
  interpreter. `hle_func_if` calls HLE only for exact audited action shapes;
  `AR_ACTION_ROOM_GFX_HLE=0` and `AR_ACTION_ROOM_LOAD_HLE=0` are the native
  controls, while title/SIM raw selectors, BG3, wrong DB/M/X, bad asset sizes,
  and unknown operands take the still-generated ROM body. The command-5/4
  matrix stages 166,496 oracle bytes with zero mismatch. A same-binary graphics
  A/B executes 39 command-7 plus 36 command-6 HLEs (320,000 bytes) and matches
  all 204 complete artifacts exactly.
