...
| **Action BG semantic roles + per-layer extents (2026-08-10)** | `ActionBgPlan_Build` classifies playfield/scene/backdrop and available/fixed L/R/T/B caps → `ActionBgPlan_CanvasOwner` / `PrimaryLayer` select finite horizontal and vertical capture owners → `ActRaiserActionBg_ApplyPlanExtents` seeds PPU row caps → immutable `FrameSlot` plan drives Diorama UV spans and debug guides | host policy only; authentic pixels and emulated state are inviolable | "let the playable world grow without repeating a finite landmark backdrop" | all 49 action map IDs; Bloodpool `136..224`; Death Heim `144..224`; Kasandora world `256..512`; `0708` scene exception | 🟢 Ordinary BG1 remains wide while each tuned Bloodpool backdrop follows its safe span: `0201` uses 76/100 with independently available water, `0202` uses 68/68 with its repeat-safe water inheriting the same bound, unbanded `0206` uses 68/68, and unbanded `0207` uses 92/92. Boss room `0208` deliberately keeps its provider-backed BG1 world but changes its independent edge/motion to Mirror/fill with a 16/16 cap; viewport BG2 uses Mirror/fill at 0/0. The provider now accepts any validated world-source edge, and the fitted camera limits each side to the playfield's effective fixed cap instead of the larger display budget. A band that owns authentic row 0 or 223 carries fill, motion and extent into the adjacent synthetic vertical margin; internal bands remain bounded. Kasandora `0301/0302` stores its dune seam once in world coordinates and resolves it through live BG2 parallax. Settings → Layers → BG Extents can add/delete up to four bands per BG and tune anchor, interval, fill, motion, and extent, plus independent non-destructive side/vertical-bound bypasses. Mixed anchors are proven disjoint across full camera travel and stale drafts retain the canonical plan; promoted room policies live in one source/edge-guarded data table. The scene inspector uses the PPU's shared fill/motion/source-X resolver. Multi-band, world-anchor, migration, capacity, Diorama-span, provider-edge, and real-PPU motion-phase regressions pin the seam. No new ROM symbol was required; this is a confirmed host presentation seam. |
...
| **Action scene accent observation/presentation (2026-08-10)** | game-thread `ActionSceneEffects_CaptureFrame` → immutable `FrameSlot.action_scene_effects` → pure bounded `ActionSceneEffectRender_Build` → the same pre-HUD world-overlay submission as spells | read-only WRAM/BG-map observation plus capability-checked SDL additive untextured geometry; no new shader artifact or emulated write | "attach portable light/particles to exact authored torches, enemy/player projectiles, trap lightning, and boss lightning" | Bloodpool BG1 torch metatile pair `$47/$4F`; fireball handler/resume/state plus `$17/$45EF` or `$18/$4610`; trap source/resume/state/handler plus `$1F/$46FE` or `$20/$479D`; map-$08 boss source `$BDFF`, handler `$8661`, `$7E:5000` states `$02-$07` with exact visual/composition pairs, state-`$09` impact pairs, and a validated live-parent backlink `+$3A`; global sword beam handler `$9D1C`, animation `$06:8000`, attacker/player link, no V-flip, pairs `$13/$30/$99E8` or `$14/$31/$9A17`, and four explicit signed-origin OAM rectangles; shared bounded `ActionBgMapView` | 🟡 Hook and renderer are implemented for flat and Diorama modes. Torches project on compositor-published BG1-low; actors project on OBJ priority 0. Diorama's hidden OBJ apron remains explicit. Slot continuity combines source/resume, boss source/backlink, or sword source/player-link identity with bounded motion, and geometry writes directly into final bounded batches. Exact captures cover Bloodpool maps `$03/$05/$08`, both fireball directions, trap handler transition/full 176px extents, run `20260810-180202`'s six vertical/diagonal long/medium/short boss strikes, and sword runs `20260810-175403`/`20260810-184935`/`20260810-190012`/`20260810-190729`. Visual `$20` is intentionally blank; the separate boss impact remains floor-only. Boss filaments follow up to 24 exact OAM-row segments; the sword uses full-height 80px/56px haze and forty-eight fixed crossed stars at sixteen top/centre/bottom cross-sections from 4px to 88px behind either direction. Independent alpha/scale phases materialize each glint without moving its centre. One expanded stream is explicitly budgeted and duplicate beams fail closed. Fresh post-fix visual acceptance remains before promotion to 🟢. |
