...
history, and project acceptance belong in their dedicated documents and should
be linked rather than repeated here.
...
| Seam | Routine / address | Hardware | Intent | Logical ID / table | Status |
|---|---|---|---|---|---|
| **Vertical-extension per-layer bounds** | `ActRaiser_ApplyVerticalMarginPolicy` → `PpuSetVerticalMarginLayerClip` | PPU BG scanout and captured action layers | "show real rows above or below the native viewport without wrapping a finite layer" | independent BG1/BG2 top/bottom availability | 🟢 Symmetric capture and per-layer clipping are implemented; geometry and evidence live in `rendering-engine.md` §13i. |
...
| UI/dialog tilemap compose+upload (sim engine) | Interactive `$01:8E29` and fixed `$02:BF60` have distinct grammars and write BG3 `$7F:B000` → `$02:AEEB`; box artwork remains BG2. `$02:C1B7` erases individual record footprints; `$01:8CCE` and `$02:ABC4/$BA41` clear larger ranges. Sky Palace setup `$02:B6F8-$B726` conditionally copies ROM `$07:D0A0` to `$7E:C200`. | BG3 text plus WRAM/VRAM BG2 map state | preserve native controls, retained-row scrolling, and independently owned menu lifetimes | Fixed composer receives packed row/column in A, saved in caller-specific DP `$14`; not a global message-type ID | [Dialogue decompilation reference](dialogue-system.md) maps source selectors, reader-return observation (including internal dictionary loops), native control barriers, authored page/wait scheduling, and partial clears. Whole-map BG refreshes and palace margin reconstruction remain separate rendering operations. |
...
| **Action HUD / stage cards / title options** | Existing `$02:BF60/$C1B7` observations; action template `$02:8E7E`, ENEMY writer `$00:A4D6`; title mode/screen HDMA `$02:A92F` | Known BG3 cells plus palette snapshot, no native gameplay writes | Preserve native input/timing, arrows/icons/health, coloured bands and italic counters | 90 fixed routes including dormant sound test; frame ABI 27 / raster request ABI 12; palette/italic participate in cached font identity | [Action/title text reference](dialogue-system.md#action-hud-cards-and-title-options). The lower title band is Mode 1 BG3, distinct from the Mode 7 logo. |
| **Credits pages** | Scene 08/01 asset loader → `$7E:4000-$DFFF`; `$02:AB30` → `$7F:B000`; VRAM font word `$5000` | Captured page map matched to its native resident producer | Replace bounded Unicode pages without changing native fades, holds, completion marker or input | One atomic 32×26-cell grid; up to six fitted lines; native palette initial accent; copyright pages excluded | [Credits adapter](dialogue-system.md#graphical-credits-page-adapter). Restores, clears and scene switches fail closed; warm pages reuse text and raster caches. |
...
| **Mode 7 world navigation** | native focus/matrix/zoom/location state plus classified label/plaque/Palace OAM → immutable world scene and frame ABI 27 screen text | PPU Mode 7 plus host curved-relief renderer | "move, zoom, spin, select a localized destination, and enter an action stage" | developed map, focus, affine matrix, registered town relief, mountain ranges, shared authored town models, oblique camera, localized `city.*.name`, native plaque/Palace and water | 🟡 Navigation behavior and ownership remain confirmed. Enhanced text replaces only the variable native glyph prefix; the authentic plaque and Palace retain independent captures. Screen records use authentic-pixel bounds rather than leaking OBJ through the BG3-cell ABI, support logical-leading RTL, live OBJ colors, pack switches, cached shaping and native fallback. Model embedding, aligned plains, graded boundaries, optional native-resolution ground, shared native mountain meshes and full-globe cloud cover are implemented. Native cliffs use owned corners and closed walls, not the shared flat grid. Mountain-boundary geometry, camera matching, live palette effects, unrestricted globe inspection and descent remain open. See `rendering-engine.md` §13h. |
| Brightness / forced blank | `$2100` INIDISP | PPU | "fade in/out, blank during build" | 4-bit master brightness + forced-blank bit | 🟡 World navigation now remains enhanced for every non-forced-blank brightness step: host layers receive one exact black overlay `(15-brightness)*17`, native Palace/plaque captures are already shaded, and enhanced label text receives the same draw-time tint. Forced blank still fails closed to authentic black. Other presentation modes remain to audit. |
...
- environmental winner masks select the source's owning screen per scanline:
  TM is preferred, otherwise TS is resolved with the same priority rules; this
  lets a flat BG1-stage effect retain authentic TS-only OBJ occlusion;
...
PPU hardware state, while the additive/winner masks, virtual face plane, and
capture flags are host-only frame metadata.
...
| World-navigation immutable frame | `SimRenderMetadata_CaptureFrame` → `SimWorldNavigationCapture_Capture` → `SimFrameData.world_navigation_scene` | `AR_SIM3D_WORLD_NAV` independently selects `$18/$19=00/09`; `town` remains zero. The game thread captures native focus, matrix, zoom, location and brightness plus all enabled towns' shared voxel identities. Presentation preserves those controls while rotating a fixed stereographic continent on a closed sphere, 2048px reconstructed developed art, and actual town models through the shared cache/palettes/proportions and D32 pass. Terrain registers town datums with offsets `{0,3,4,4,0,4}`, blends four cells either side of boundaries, pins ocean-connected water, and adds connected mountain relief from the world rock palette. A separate geography serial prevents wave animation from rebuilding relief. Model and ground enablement both follow `$7F:6B18`: stale structures after a progress reset must not render over an undeveloped stamp. Populated-save replays use the non-editing world-navigation settings fixture. Haze/weather share the surface; label glyphs, plaque and Palace retain separate OAM ownership, and malformed captures fail closed to authentic Mode 7. Native full-town canvases, unrestricted orbital inspection and seamless descent remain open; details and evidence are in `rendering-engine.md` §13h. |
| World-navigation native ground | `SimWorldNavigationTownGround` → `SimWorldNavigationArt_OverlayTownGround` + `SimTownGroundArt` | Game-side capture owns all enabled towns' row-major retained terrain, native development tier and model-source masks; presentation receives values, not WRAM/PPU pointers. The portable art provider copies bounded immutable ROM bytes at startup and lazily decodes four native character/palette variants. The existing 2048px atlas receives exact cell-aligned ground with four-cell boundary feathering; masked models receive clean ground/river and disabled models retain overview glyphs. Mountain identities are excluded; cliff artwork activates only with owned-corner geometry. The independent native mountain stage replaces accepted mountain footprints with clean ground beneath raised geometry. Native water/waterfalls use lazily cached four-phase CHR variants from the captured eight-tick clock, without live VRAM/DMA reads. Source/tier/toggle/phase changes invalidate the atlas even when the Mode-7 serial is unchanged; camera and model-pose changes do not. `sim3d_world_navigation_ground_detail` is live and independent of SIM-town settings. Missing art leaves the prior overview intact. Live palette effects, boundary polish and descent remain open; fallback checkpoints and replay evidence are documented in `rendering-engine.md` §13h. |
| World-navigation native cliffs | `SimWorldNavigationTerrain_TownCellCorners` → `SimWorldNavigationCliffs` → cached presentation | Sparse caps retain both sides of authored hard edges while applying the existing world registration, coast and boundary grading. Clipped skirts close each exposed height interval using the town's crossing-edge rule and face/cave-jamb material policy. The six stock towns need only 167 caps and 155 skirts. All ground passes replace the corresponding shared-grid cells: opaque D32, cloud shadows, blur and haze use identical projected vertices; model anchors use the owned caps. Native cliff pixels use the existing ground atlas, with no new material pass. Detailed ground plus nonzero relief gates geometry/art together, and missing art/resources or disabled towns preserve the overview. Geometry and spherical UV caches are independent of camera and water animation. Immutable portable data stays above the renderer boundary. Tests and pixel-exact Off/On replay evidence are in `rendering-engine.md` §13h. |
| World-navigation native mountains | immutable retained terrain → `SimWorldNavigationMountains` + `SimBackgroundMountainMesh` | The portable local mesh emitter is shared with town rendering, including inclined faces, tapered layers and fitted silhouette walls. Navigation adds silhouette-fitted rear slopes inside the original footprint, clipped against all retained town ground/model masks so buildings and other non-mountain cells remain clear. Existing front/wall vertices are unchanged. It uses audited source stamps and Low geometry, projects through the fixed globe metric and owns a separate `WorldMountain` atlas/material so an active town cannot overwrite its textures. Only accepted footprints suppress inferred relief and remove old perspective stamps. `SimWorldNavigationMountainTransition` blends nearby inferred ridge scales and categorical rock colours toward native heights/palettes over 12 cells, with exact fallback-town preservation and a four-cell exterior fade. Sparse retained material patches add no depth draw; water, snow, sand, structures and town floors are not reclassified. Unknown topology or resource failure retains the overview fallback; disabling the live stage restores it. Camera/water changes do not rebuild sources or transition fields; geography changes do. Shared geometry does not yet imply matched cameras, final boundary geometry or seamless descent. |
| World-navigation full-globe weather | fixed spherical normals → `SimWorldNavigationClouds` → retained shell/ground projections | Three deterministic 3D-noise banks have identical longitude endpoints and constant pole rows. Rigid spherical rotation drives drift; padded longitude sampling unwraps small quads. Cloud bodies cover the visible tangent cap of a complete concentric sphere, with depth-tested shadows over both mapped terrain and the complete ocean. No map-bound fade or rectangle limits cloud coverage. Backside ocean-shadow culling relies on the opaque planet; full-world weather remains independently switchable from its shadows, lighting and atmosphere. CPU geometry and portable material IDs stay above the backend boundary. |
...
| Structure animation and progress identity | `sim_structure_visuals.c` + `ApplyStructureFrame` + `SimBackgroundVoxels_Classify` + `SimBackgroundVoxelObject::visual_state/animation_phase` | Every native frame owned by an enhanced replacement is catalogued from the `$03:D4D2/$D4E2` step-program trees and resolved by matching the plot's live 2×2 tile entries against the structure atlas at `$7E:3100`. House class 0 contributes eight rows of shared two-stage scaffold plus two completed facings (`$00-$3B` in eight-byte rows); bridge class 2 contributes stone `$4C/$4D -> $44/$45` and Northwall ice `$EA/$EB -> $E2/$E3`; windmill class 6 contributes scaffold `$04/$06/$14` and blade cycle `$24/$26/$16`; factory class 8 contributes `$34 -> $36`. Record flags and `$E1/$E2` bridge cell markers identify objects but never their appearance. Construction is resolved before regional house dispatch, so all six towns share the scaffold path. An unmatched owned plot is recorded in `SimBackgroundVoxelScene::unmatched_visuals` and omitted from enhanced replacement, leaving authentic art visible instead of guessing "finished"; `AR_SIM_STRUCTURE_DEBUG=1` reports every resolved and unmatched transition (`AR_WINDMILL_DEBUG` is an alias). The one intentional divergence remains the Extras toggle "Wind stops every windmill", which holds every mill while any town record carries the story bit. The contract test enumerates every catalog entry, crosses every house frame with 6 towns × 3 levels × 2 facings through classification and model compilation, exercises both bridge axes and ice variants, and includes the exact Aitos gf14690 snapshot seam. |
...
| Action OBJ apron (resolve channel) | `action_obj_apron` + `ActRaiser_DioramaApronFinish` | resolve-only OBJ surface outside the displayed span | "give edge shaders complete sprite samples without widening real OAM or the visible world" | exact host parts, shared OAM-order z-test, and fixed resolve bands | 🟢 The apron is never displayed and changes no shader-off pixel; full geometry and invariants live in `rendering-engine.md` §13j. |
...
| Cull cover eligibility | `Sim3D_SourceCullCover` (`src/sim/sim_render_metadata.c`) | Only sprite-window rejection creates cover; current eligibility and placement rules live in [the renderer cull-cue contract](rendering-engine.md#13d-simulation-town-cull-cues-2026-07-22). |
...
| Lit-window lift inset | `Sim3D_MaxDrawLift` (`src/sim/sim_render_metadata.c`) | The bottom boundary uses the classifier's maximum draw lift; the geometric rationale lives in [the renderer cull-cue contract](rendering-engine.md#13d-simulation-town-cull-cues-2026-07-22). |
...
| Sim camera mode | `Sim3DCamera_CapturePresentationState` (`src/sim/sim3d_camera.c`) | Free and Dynamic each own a pose — Free the player-authored `sim3d_tilt_*`/`distance`, Dynamic the dedicated `sim3d_dyncam_baseline_*`. The resolved pose is published through `sim.projection_*`. Dynamic right-drag adds a temporary returning orbit, without rewriting the Free pose; "Reset camera" restores the active mode's pose. |
| Globe inspection camera | `Sim3DCamera` + `SimWorldNavigationGlobe_OrbitFrame` | Navigation owns visit-local orbit/zoom and an explicit inspection-focus blend independently of both persisted town poses. Host camera actions publish this through application-owned `FrameSlot` fields, including retained-frame refresh while paused; no runner ABI or persisted settings change. Presentation rotates the globe frame and reframes the actual camera matrix/eye together, never native travel coordinates. Town selection uses projected size/viewport and conservative whole-model horizon bounds, not the native destination label or a zero/nonzero orbit switch. |
...
identity is established before the first runner tick, and replay sessions never
persist SRAM or settings. `AR_LOADSTATE` is rejected at startup: raw runner
snapshots omit the recompiled CPU and suspended game continuation. Host load
actions also fail before touching RAM; capture remains a debug inspection tool.
The separate `snesref` oracle adapter
...
- Each command byte dispatches on its **highest set bit**, MSB first:
  ...
  |---|---|---|
  | 7 | `$02:B28E` — VRAM char/tile upload. Guarded action-only HLE for audited compressed 8 KiB banks and 4 KiB font; raw/non-action/unexpected shapes retain native | 6 |
...
- The action-room command-7/6/5/4/3 CPU handoff is live without replacing this
  interpreter. `hle_func_if` calls HLE only for exact audited action shapes;
  `AR_ACTION_ROOM_GFX_HLE=0`, `AR_ACTION_ROOM_LOAD_HLE=0`, and
  `AR_ACTION_ROOM_VIDEO_HLE=0` are the native controls, while title/SIM raw
  selectors, BG3, wrong D/DB/M/X, bad asset sizes, and unknown operands take
  the still-generated ROM body. The command-5/4 matrix stages 166,496 oracle
  bytes with zero mismatch. Same-binary graphics and video A/Bs execute 39
  command-7 plus 36 command-6 HLEs (320,000 bytes) and 12 command-3 HLEs (336
  bytes), respectively; each matches all 204 complete artifacts exactly.
