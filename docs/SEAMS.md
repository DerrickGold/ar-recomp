...
| World-map underlay | `src/sim_world_map.c` + `src/sim_world_map_build.c`, drawn by `DrawSimWorldUnderlay` (`src/present.c`) | Reads the flat ROM base tilemap `$06:B341`, 256 8bpp tiles `$0E:8000`, and palette `$1C:BF93`. On town entry and development-input changes it transactionally seeds `$7E:C000`, forces `$19=09` and pointer bank `$AA=7E`, invokes only the bounded `$02:865C` overlay closure (`$02:86D1/$02:8726`), copies out the complete developed tilemap, then restores CPU, all WRAM, and math-unit state. It never adopts the live shadow: acts durably clobber rows 0-79 and towns reuse rows 0-7. The overlay is separable from `$02:B475`'s base load and `$2118` VRAM upload, has no yield/hardware path, and reads sim state from `$7F:2000-$37FF`, `$7F:6B18-$6B23`, and `$7F:9101` bit 0. One world tile = one town cell, so the map draws at 2x |
