...
| **Mode 7 world navigation** | native focus/matrix/zoom/location state → immutable world scene | PPU Mode 7 plus host curved-relief renderer | "move, zoom, spin, select a destination, and enter an action stage" | developed map, focus, affine matrix, registered town relief, mountain ranges, shared authored town models, oblique camera, Palace/UI and water | 🟡 Navigation behavior and ownership remain confirmed. Model embedding, aligned plains, graded boundaries, optional native-resolution ground, shared native mountain meshes and full-globe cloud cover are implemented. Native cliffs use owned corners and closed walls, not the shared flat grid. Mountain-boundary geometry, camera matching, live palette effects, unrestricted globe inspection and descent remain open. See `rendering-engine.md` §13h. |
