...
4. **Capture VRAM/CGRAM-dependent OBJ pixels at their authentic raster time.**
   ...
   the relevant rows. Post-scanout sampling can see art already streamed for the
   next frame; on the game-over return to Sky Palace, pre-scanout sampling saw
   the previous all-black icon state even though the IRQ restored the art before
   rows 11-26. `ActRaiser_DioramaHudObjPrepare` therefore resolves the footprint,
   `…CaptureLine` retains each row immediately after `ppu_runLine` displays it
   and before HDMA/IRQ advances, and `…Finish` publishes after scanout. A caller
   whose source state is proven stable may still rasterize once before scanout;
   that is a source-specific fact, not an overlay-capture API rule.
...
range and footprint before scanout, retain its pixels row-by-row at the same
raster state that displayed them, publish it to the flat overlay surface after
...
`ActRaiser_DioramaHudObjPrepare`/`…CaptureLine`/`…Finish` implement this split.
The best-effort restore is still sampled before scanout and does not replay the
hardware per-line sprite limits, so it fails toward showing a sprite the PPU
would have dropped rather than toward a hole.
