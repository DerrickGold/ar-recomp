...
| **Action scene accent observation/presentation (2026-08-10/12)** | game-thread `ActionSceneEffects_CaptureFrame` → immutable `FrameSlot.action_scene_effects` → pure bounded `ActionSceneEffectRender_Build` → the same pre-HUD world-overlay submission as spells | read-only WRAM/BG-map observation plus capability-checked SDL additive untextured geometry; no new shader artifact or emulated write | "attach portable light/particles to exact authored torches, lava surfaces, enemy/player projectiles, trap lightning, and boss lightning" | Bloodpool BG1 torch pair `$47/$4F`; Marahna `$05/$04-$08` single-metatile `$43` torches with camera-local publication; Aitos `$04/$01` `$DC/$DD+/$DE` lava rims over `$DF`; map-backed styles use an observer clock seeded from `$0088` and subsequently advanced only by the `$00:8C98` gameplay-pass serial; Bloodpool, complete `$E047` Marahna orb/split, parent-validated `$DE96` Marahna snake shot, and `$CF9E/$CFCD` Aitos fireball lifecycles; Bloodpool trap pairs; Marahna `$4AA1/$4B82` connector children validated against their `$E18E/$E254` endpoint pair, backlink, orientation, and midpoint; Bloodpool map-$08 boss source `$BDFF` states `$02-$07` plus state-`$09`; Marahna boss source `$E483` exact charge/orb/diagonal-bolt/ground-charge phases and validated `$12E0` backlink, including `$E578` descent and `$E57E` three-frame floor lifecycle; global sword beam handler `$9D1C`, animation `$06:8000`, attacker/player link, no V-flip, pairs `$13/$30/$99E8` or `$14/$31/$9A17`, and four explicit signed-origin OAM rectangles; shared bounded `ActionBgMapView` | 🟡 Hook and renderer are implemented for flat and Diorama modes. Torches and lava surfaces project on compositor-published BG1-low; actors project on OBJ priority 0. Diorama's hidden OBJ apron remains explicit. Native pause/freeze leaves the gameplay serial unchanged, freezing actor clocks and the shared map-backed clock even while emulated vblanks continue. Slot continuity combines validated lifecycle keys with bounded motion; Marahna and cyclic Aitos fireballs reset on immediate same-slot teleport/relaunch reuse. Geometry writes directly into final bounded batches. Runs `20260811-221433`, `20260811-225534`, and `20260811-232640` correct `$34/$4BE5` to moving-platform machinery, map the complete `$E047` left/idle/right orb cycle and four correctly flipped cardinal children, add the actual `$DE96` snake projectile while rejecting the reaper's `$E0BA` orb, confirm ten `$43` boss-room torches, and map the boss's complete `$57C2/$5868/$59DE/$5CE0/$5D01/$5D0D/$5D2E` electrical cycle. Bloodpool boss filaments follow up to 24 exact OAM-row segments; Marahna reserves five two-layer ten-segment links and one two-layer eight-segment diagonal boss bolt, while the ground charge stays within the ordinary bounded glow/particle budget; the sword uses full-height 80px/56px haze and forty-eight fixed crossed stars. Impossible duplicate/sixth expanded streams fail closed. Production flat/Diorama projection regressions cover separate BG1 and OBJ planes. Fresh post-fix visual acceptance remains before promotion to 🟢. |
