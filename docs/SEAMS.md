...
| **Action OAM pipeline (fully mapped 2026-07-10)** | `$00:8C98` per-frame cull (obj vs 256×224 window keyed on camera `$22/$24`; sets/clears offscreen bit `$0400` in obj+`$30`) → `$00:8D68` sprite builder (walks 7-byte sprite defs at bank obj+`$18`, ptr obj+`$20`+5; draw window x∈[-16,256) y∈[-16,224) authentically, both axes now widened by the live margins — x by `extraLeftCur/extraRightCur`, y by `extraTopCur` (diorama vertical extend); writes `$0380` shadow + packed high-table bits via `$00/$9A/$9C`, plus an exact per-slot signed Y to `PpuSetObjYOverride`) → `$00:923A`→`$9258` **HUD sprites** (from the `$06:A800` bank-6 table: FIXED screen positions, no camera math, no cull — widescreen-safe/centered) → OAM DMA `$02:ACA6` (544B `$0380`→`$2104`) | OAM | "which objects are visible + build their hardware sprites" | see ram-map.md "OAM shadow + sprite-build working vars" | 🟡 Stages C/D1/D2 and the combined wide path are directly validated across every action level in regions `$01-$07`, including the complete Death Heim flow. Wide activation is default-on; `AR_WS_MARGIN_ACTIVATION=0` is the fidelity gate. **The OAM Y field is a lossy encoding** — 8 bits mod 256 against 224 lines, so a parked slot, a sprite off the BOTTOM and a sprite above the TOP are indistinguishable in it. `PpuSetObjYOverride` carries the emitter's un-truncated value alongside, and above-screen scanlines draw only from slots that have one; that is the seam that made a vertical band possible at all. The override outranks the OAM byte on EVERY line, which makes the sideband strictly action-owned: `ActRaiserDrawPpuFrame` clears it on each non-action frame (ledger §34 — stale overrides once resurrected parked slots as garbage sprites in the first sim cutscene after an act). The vertical DRAW window is widened, activation is deliberately NOT — that stays on the authentic window because it is game logic. See rendering-engine.md §13i. |
