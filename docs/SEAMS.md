...
| **Immutable action-room scene authority (2026-08-22)** | `ActionRoomScene_Load` replays cumulative `$05:8000` graphics commands and command 3; `ActionRoomScene_BuildFrameState` resolves `$02:893E + 28*profile`, native camera/parallax, CHR/page phases, the inherited `$7E:6000-$7FFF` raster workspace and R1-R10 per-line state; `ActionRoomScene_RenderNativeFrame` composites the stable Mode-1 BG1/BG2 scene. Editor exporter schema v4 emits that state plus per-room C golden hashes and additive first-visible raster-camera metadata. `AR_ACTION_ROOM_SCENE_COMPARE=1` shadows both each live `ActionBgWorld` publication and the pre-scanline PPU registers after HDMA advances. `AR_ACTION_ROOM_STAGE_COMPARE=1` independently compares the command-4/5 dimensions/map/metatile WRAM image before layer-enable checks. The immutable page maps/metatiles are the default finite-provider source while the live ring remains active; `AR_ACTION_ROOM_SCENE_HLE=0` selects the staged-WRAM source control | immutable ROM bytes plus host-owned value records; comparison uses host-isolated live worlds and is read-only to provider/PPU/game state; the provider changes only its tile-word source and automatically falls back to live WRAM on scene/shape failure | "load any stable action room without a save state, keep editor captures identical to the game's HLE staging, and validate the port against the live PPU" | 49 rooms render through the stock-ROM census; 49 pinned profiles, 30 character-animated rooms, `0402/0403` four-page cycle, 17 raster-bearing rooms, five forced-BG2-priority rooms; 12 ordinary-entry routes compare all 24 loaded layers and 166,496 staged WRAM bytes; same-binary command-7/6 and command-3 A/Bs each match 204/204 artifacts | 🟡 Stable BG1/BG2 authority, JavaScript golden-frame self-check, camera-local Native/Diorama editor surfaces, frame-register comparator, ROM-free tests and stock-ROM render census are implemented. Live R1-R10 acceptance includes 7,049,476 exact registers from the original matrix, 250,638 exact natural R4 registers, 4,055,368 exact Northwall-route registers, and 16,525,173 exact Death Heim-route registers. Natural fixtures pin R4's low-byte phase/flat entry table, Northwall's R6 bootstrap camera and R7 entry, every Death Heim rematch bootstrap camera, provisional-to-stable BG2SC, the post-boss `$64/$74` page handoff, and 1,351 frames retaining the final R8 table. Northwall and Death Heim same-replay default/native presentation controls are byte-identical for every framebuffer, PPU snapshot, and final emulated-state artifact; Death Heim's raw dispatch diagnostic differs only by native/HLE ordinal accounting. A separate 6,227-frame natural Fillmore route adds zero-fallback/zero-mismatch production evidence and exposed a now-fixed diagnostic/provider cache alias. The ROM-derived provider-source handoff is default-on after its exact 12-target matrix and natural Fillmore acceptance; native ring preflight/fallback remain active. Guarded CPU HLE now stages action command-7/6 CHR/CGRAM, command-5/4 maps/metatiles and command-3 video profiles while preserving generated native fallbacks; the graphics arm executes 39+36 commands/320,000 bytes and the video arm executes 12 commands/336 bytes across exact 12-target matrices. Commands 2/1/0, gameplay staging, actors, callbacks, audio and transitions remain native. BG3/OBJ composition, fades and gameplay-object-driven windows remain outside the stable-room contract. The generated editor lives under ignored `build/action-editor/`. See `action-room-loader-hle.md`. |
