...
| **Immutable action-room scene authority (2026-08-22)** | `ActionRoomScene_Load` replays cumulative `$05:8000` graphics commands and command 3, resolves `$02:893E + 28*profile`, expands finite BG1/BG2 tile words with `$ECFF` plus profile common priority, and exposes pure CHR phase/BG2 page/raster identity; editor exporter schema v2 and game boot both register this module; `AR_ACTION_ROOM_SCENE_COMPARE=1` shadows every newly published live `ActionBgWorld` | immutable ROM bytes plus host-owned value records; comparison is read-only and never gates the provider | "load any action room without a save state, and keep editor assets identical to the game's HLE staging" | 49 rooms, 49 pinned profiles, 30 character-animated rooms, `0402/0403` four-page cycle, 17 raster-bearing rooms, five forced-BG2-priority rooms | 🟡 Shared asset/profile/phase authority and full-world tile comparator are implemented and ROM-free tested; the stock-ROM census passes. The generated editor lives under ignored `build/action-editor/`. Remaining work is the shared 256×224 scanline compositor: ten raster builders, live-equivalent scroll/parallax, TM/TS/windows/mosaic/colour math, and golden surface/final-composite hashes. See `action-room-loader-hle.md`. |
