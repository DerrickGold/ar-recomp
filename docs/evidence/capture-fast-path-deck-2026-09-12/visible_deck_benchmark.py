...
def summarize(log):
    ...
    for block in log.split('[pipeline-perf]')[1:]:
        ...
        if not header.startswith(' scene=Action 3D ') or ' map=04/04 ' not in header:
            continue
...
def main():
    ...
    manifest = ROOT / 'tests/fixtures/benchmark/action-render-checkpoints.json'
    fixture = json.loads(manifest.read_text())['checkpoints']['aitos-diorama']
    settings = ROOT / fixture['settings']
    seed_file = ROOT / fixture['sram_base64']
    seed = base64.b64decode(seed_file.read_bytes())
    ...
    env.update(AR_HEADLESS='0', AR_HEADLESS_VIDEO='0', AR_INPUT_REPLAY=str(replay),
               ...
               AR_QUIT_FRAMES='3000', AR_ENABLE_RUN_DIR='1', AR_PIPELINE_PERF='1',
    ...
    for i, variant in enumerate(args.order):
        ...
        if not frame or int(frame[1]) != 3000:
            raise ValueError('Did not complete exactly 3000 emulation ticks')
