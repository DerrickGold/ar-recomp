...
| Address | Size | Description |
|---------|------|-------------|
| $7E:035A | 1 | Event-effect request port: COP vector `$00:8526` stores A's low byte here (`LDA #id; COP`). Consumed by the NMI tail `$02:AC33` every other frame as the LOW byte of one 16-bit load, forwarded to APU port `$2142`, then zeroed. The serial replay corpus covers `$01/$07/$12/$83/$85/$89/$8A/$90/$94/$9A/$9E/$A0/$A1`; high-bit ids duplicate their low-seven-bit sequence across both native effect lanes. See [snes-native-audio-channels.md](snes-native-audio-channels.md) for allocation and loss semantics. |
| $7E:035B | 1 | SFX request port: BRK vector `$00:852F` stores A's low byte here (`LDA #id; BRK`). Forwarded as the HIGH byte of the same 16-bit NMI store to APU port `$2143` and zeroed together with `$035A`. **id `$00` = idle/clear, not a sound** — it is by far the most-written value in the early census (754 posts vs 12 key-ons, mostly from `$03:9E6B`), and the NMI forwards zero as "nothing pending". The later serial trace confirms exposed BRK ids `$02/$03/$09/$0C/$10/$1A/$1B/$20/$21` across its staged replay corpus; the complete static post catalog, COP/event ids, sample mapping, and per-id loss ranking live in [snes-native-audio-channels.md](snes-native-audio-channels.md). |
