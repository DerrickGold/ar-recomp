...
| SNES address | File range | Meaning |
|---|---:|---|
| `$00:8C98-$8D67` | `0x00C98-0x00D67` | Action OAM rebuild/cull. Clears the shadow, calls the fixed-HUD emitter, scans action objects, updates authentic visibility/activation state, and calls `$00:8D68` for each admitted composition before returning through one common epilogue. The native pause/freeze path skips this routine while vblanks continue, so its completed-call cadence is the verified presentation clock for host action lighting and particles. The serial itself is host state, not ROM or WRAM data. |
