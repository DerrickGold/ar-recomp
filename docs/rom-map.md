...
| `$01:8491-$84B1`, `$01:A076-$A086` / JP `$01:8458-$8478`, `$01:A045-$A055` | `0x08491`, `0x0A076` / `0x08458`, `0x0A045` | Take Offering's held-capacity gate versus town-grant insertion wrapper. Full held inventory blocks all collection, including Western auto-activated Sources. Full town inventory returns failure; the five checked Source discovery owners ignore it after setting their one-time guard. [Capacity controls and limits](regional-differences-technical.md#full-inventories-and-one-time-discoveries). |
...
| `$03:F3AB-$F3EF` / JP `$03:EE87-$EECB` | `0x1F3AB-0x1F3EF` / `0x1EE87-0x1EECB` | Northwall lake search shares Marahna's `$7F:9173` but does not reset it. Equality with 255 grants item 4, sets fired 3, clears dispatched 3 and requests message 29 through `$9D`. Scene record `$03:E5E2` / JP `$03:E0E1`. [Counter and native-scene controls](regional-differences-technical.md#northwalls-lake-search-shares-marahnas-counter). |
| `$01:8105-$810A` / JP `$01:80FE-$8103` | `0x08105-0x0810A` / `0x080FE-0x08103` | Palace initialization writes `$01` to `$7F:9102` in all five ROMs. Clears fishing-init masks `$40/$20` without deleting town Compass knowledge or completion; unfinished Fillmore/Marahna callbacks reset their counter on re-entry, whereas Northwall retains it. [Live excursion and source evidence](regional-differences-technical.md#source-discoveries-and-compass-fishing). |
...
| `$03:E8EF-$E926` / JP `$03:E3E3-$E41A` | `0x1E8EF-0x1E926` / `0x1E3E3-0x1E41A` | Fillmore event14 grants Magic Skull and enables15; event15 requests message `$8F`. Both slots share regional discovery text. Original-selector controls pass in all five ROMs, but event14's normal trigger is unidentified. [Ownership and limits](regional-differences-technical.md#fillmores-additional-magic-skull-event). |
| `$03:EC02-$EC7C` / JP `$03:E6EE-$E768` | `0x1EC02-0x1EC7C` / `0x1E6EE-0x1E768` | Kasandora event0 requests prosperity message26 through pending `$9A` while unfinished, act count !=2 and prerequisite8 clear. Slots25/26 share this text despite empty callbacks. [Message-owner audit](regional-differences-technical.md#dialogue-only-slots-and-indirect-message-owners). |
| `$03:EEE1-$EF2D` / JP `$03:E9C5-$EA11` | `0x1EEE1-0x1EF2D` / `0x1E9C5-0x1EA11` | Aitos event2: tests global26 but calls town setter at `$EEEC` / JP `$E9D0`, writing fired26 through inherited Y. Scene record `$E664` / JP `$E163`; counter `$9171`, target128, timeout message11. [Flag mismatch and test limits](regional-differences-technical.md#aitos-dying-man-scene-flag-mismatch). |
| `$0A:DB0C-$DB16` / JP `$04:AB93-$AB9D` | `0x55B0C-0x55B16` / `0x22B93-0x22B9D` | Aitos injured-man program: alternate pose commands13/12, wait120 actor updates after each, loop with signed displacement-11. Compositions `$01:E85C/$E892` (JP `$E7E6/$E81C`) use tiles `$A0/$A1`. Repeated event2 initialization suppresses the alternate pose during the tested live waiting interval. [Program and native comparisons](regional-differences-technical.md#live-scene-animation-and-rain-response). |
| `$03:B734-$B763` / JP `$03:B4BD-$B4EC` | `0x1B734-0x1B763` / `0x1B4BD-0x1B4EC` | 24 lair-reward words, four per town: each row supplies items18/20 and two `$8000` technology rewards. Not 24 inventory grants. [Complete direct-call census](regional-differences-technical.md#offering-grant-owners-and-capacity-limits). |
