...
The current ActRaiser target is C and links SDL3 only. An RmlUi-based launcher
would require C++ plus an existing OpenGL 3.3 context; ActRaiser
...
extent available. Off restores the stored cap values. Neither changes the edge
strategy, finite-world clip, or shared canvas size.
