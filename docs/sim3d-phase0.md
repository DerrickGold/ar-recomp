...
Terrain elevation is a release feature, so D7 selects
`build-release/ActRaiserRecomp` and `dev-config.ini` when no command-line
override is supplied. Build the `play` preset as described in
...
including when `--all` is used for an intentional control comparison.
