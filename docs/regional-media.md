...
below, selected through **Enemy artwork**.

Open **Settings → Regions → Art & music** for the individual choices below.
**Presets → Apply art/music preset…** sets them together. Missing resources
are explained after the option's mechanics in Details; they do not change
a requested Japanese choice to Custom.

## Install Japanese enemy artwork

After extracting the Japanese donor, select **Enemy artwork → Japan**
...
and select **Death Heim statue → Japan** in the menu above.
...
filename above, then restart. Select **Action item artwork → Europe** in the
...
After extracting the Japanese donor, choose **Follower symbols**, **Lair
symbols** and **Pyramid decoration** independently.
...
After installing the Japanese donor, select **Music versions → Japan**
...
theme. US and Europe use the Western sequences. **Stage music choice** separately
selects Fillmore Act 2's scene-to-track assignment. Neither choice changes
gameplay rules.
