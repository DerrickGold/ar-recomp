...
Two integration-quality gaps remain:

2. **Privileged audio policy is still low-level.** This is appropriate for a
   game adapter, but track names, replacement assets, loop metadata, and preview
   extraction belong in a game-owned catalogue/manifest contract above the
   runner. They should not become SNES-core concepts.
3. **Serialization remains internal.** A generic frontend or tool cannot yet
...
vertical extension/clipping, capture padding, and HUD split policy. Its
remaining concrete-layout exception contains capture/composition and developer
diagnostic work, not a missing general widescreen contract. A later
hypothetical audit should use a HiROM or coprocessor title, because another
LoROM game does not challenge cartridge mapping or enhancement-chip boundaries.
