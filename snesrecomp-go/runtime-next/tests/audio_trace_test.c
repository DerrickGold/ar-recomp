...
#define AUDIO_TRACE_TEST_WAV "runtime-next-audio-trace.wav"
#endif
...
static void check(bool condition, const char *message) {
    if (condition) return;
    fprintf(stderr, "runtime-next audio trace failed: %s\n", message);
    ++failures;
}
...
int main(void) {
    ...
    if (failures != 0) {
        fprintf(stderr, "runtime-next audio trace: %d failure(s)\n", failures);
        return 1;
    }
    puts("runtime-next audio trace: PASS");
    return 0;
}
