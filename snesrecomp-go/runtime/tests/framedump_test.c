...
static void check(int condition, const char *message) {
    if (!condition) {
        fprintf(stderr, "runtime framedump contract failed: %s\n", message);
        ++failures;
    }
}

int main(void) {
    static const char directory[] = "runtime_framedump_output";
    static const char metadata_path[] =
        "runtime_framedump_output/frame_000042.json";
    static const char binary_path[] =
        "runtime_framedump_output/frame_000042_wram.bin";
