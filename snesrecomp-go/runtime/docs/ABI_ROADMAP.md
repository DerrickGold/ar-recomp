# Runner ABI implementation record

`runtime` preserves a narrow generator-facing C ABI because emitted functions
link directly to CPU/dispatch support. Concrete `Ppu`, `Snes`, CPU, APU, and
DSP layouts remain private implementation details; the versioned runner table
is the extension and tooling API.

ABI v2 is the current versioned portable C API centered on a
...
handles, or platform graphics/audio types. This document is the chronological
record of that migration and its deferred ideas; `API_REFERENCE.md` is the
...
remain. See [`GAME_SDK_EVALUATION.md`](GAME_SDK_EVALUATION.md).
...
## Public SDK boundary cleanup checkpoint (2026-08-27)

The completed ActRaiser migration is now enforced as a source-layout boundary.
Supported headers live under `include/snesrecomp`; concrete hardware layouts,
runner singleton state, cached game-module tables, and implementation helpers
remain under `src`. Frontend bootstrap is separate from generated-code
watchdog/dispatch support, and the recompiler emits namespaced SDK includes.
The recompiler emits namespaced public headers directly. The former short
header forwarders under `src` have now been removed. Authored and generated
application source have no private-header allowlist; the boundary check derives
its forbidden set from every private runner header during each test configure.

The standalone public-header fixture compiles with only the public include root
and brings the runtime suite to 28/28. The application passes all 92 tests,
including the three host-GPU tests, and the Go recompiler passes all packages.
The 267-translation-unit Zig hermetic build also consumes the classified
public/private include roots and completes successfully.
All four replay workloads retain identical WRAM, SRAM, CPU-state, and dispatch
artifacts. Against the final-migration checkpoint, the portable suite is 1.24%
faster and the native-SIMD suite regresses 0.62%; all workload and suite results
remain within the phase gate. See
[`runner-abi-performance-gate.md`](../../../docs/runner-abi-performance-gate.md)
and [`GAME_ENHANCEMENT_INTEGRATION.md`](GAME_ENHANCEMENT_INTEGRATION.md).
...
5. [x] Remove external concrete-structure access only after all game adapters use
...
6. [ ] Stabilize reusable game-project integration contracts.
   ...
   - [x] Migrate the remaining capture/composition and diagnostic work from
     the final concrete ActRaiser frame adapter.
   - [x] Define and document the boundary between low-level audio safe points
     and game-owned semantic track/replacement metadata.

The completed cutover for item 5 is tracked in
`docs/runner-concrete-access-audit.md`. The application began this phase with
27 exact concrete-header exceptions and now passes the build-time fence with an
empty allowlist. Serialization, asynchronous pinning, language bindings, and
broad SDK fixtures are follow-up work; dirty-range or scatter/gather support is
warranted only when measurement identifies a real bulk copy in a consumer.
