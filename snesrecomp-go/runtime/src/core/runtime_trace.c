...
static void add_channel_name(const char *name) {
    ...
    static const struct Entry entries[] = {
        {"func", SR_TRACE_CHANNEL_FUNC}, {"vram", SR_TRACE_CHANNEL_VRAM},
        {"vmadd", SR_TRACE_CHANNEL_VMADD}, {"reg", SR_TRACE_CHANNEL_REG},
        {"dma", SR_TRACE_CHANNEL_DMA}, {"mx", SR_TRACE_CHANNEL_MX}, {"call", SR_TRACE_CHANNEL_CALL},
...
static void initialize_from_environment(void) {
    s_initialized = true;
    s_channels = kDefaultChannels;
    const char *path = getenv("SNESRECOMP_TRACE_FILE");
    const char *watch = getenv("SNESRECOMP_TRACE_WATCH_FILE");
    if (path != NULL && path[0] != '\0') watch = NULL;
    parse_channels(getenv("SNESRECOMP_TRACE_CHANNELS"));
    s_vram_low = env_unsigned("SNESRECOMP_TRACE_VRAM_LOW", 0u);
    s_vram_high = env_unsigned("SNESRECOMP_TRACE_VRAM_HIGH", 0x7fffu);
    s_wram_low = env_unsigned("SNESRECOMP_TRACE_WRAM_LOW", 0u);
    s_wram_high = env_unsigned("SNESRECOMP_TRACE_WRAM_HIGH", 0x1ffffu);
    const char *filter = getenv("SNESRECOMP_TRACE_FUNCTION");
    if (filter != NULL) {
        snprintf(s_function_filter, sizeof(s_function_filter), "%s", filter);
    }
    const char *low = getenv("SNESRECOMP_TRACE_HARDWARE_LOW");
    const char *high = getenv("SNESRECOMP_TRACE_HARDWARE_HIGH");
    ...
    s_ring_capacity = env_unsigned("SNESRECOMP_TRACE_RING_LINES", kDefaultRingLines);
    if (s_ring_capacity < 16u) s_ring_capacity = 16u;
    s_post_lines = (int)env_unsigned("SNESRECOMP_TRACE_POST_LINES", 400u);
    s_ring = calloc(s_ring_capacity, sizeof(*s_ring));
    if (s_ring == NULL) {
        fprintf(stderr, "[sr_trace] watch ring allocation failed\n");
        return;
    }
    s_watch = true;
}
...
static void write_event(const char *channel, const char *format, ...) {
    ...
    const uint32_t block =
        g_sr_block_ring[(g_sr_block_index - 1u) & kBlockRingMask];
...
void sr_trace_func(uint32_t pc24, const char *name, int m, int x,
    ...
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_FUNC) ||
...
void sr_trace_call(uint32_t pc24, const char *name, int m, int x,
                   int expected_m, int expected_x) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_CALL) ||
        !function_matches(name)) return;
    const int mismatch = (m & 1) != (expected_m & 1) ||
                         (x & 1) != (expected_x & 1);
...
void sr_trace_vram(uint16_t address, uint16_t value, const char *path) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_VRAM)) return;
    address &= 0x7fffu;
...
void sr_trace_vmadd(uint16_t address, const char *source) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_VMADD)) return;
...
void sr_trace_reg(uint16_t address, uint8_t value) {
    if (sr_trace_active() && sr_trace_channel_enabled(SR_TRACE_CHANNEL_REG))
...
void sr_trace_dma(int channel, uint8_t b_address, uint8_t a_bank,
    ...
    if (sr_trace_active() && sr_trace_channel_enabled(SR_TRACE_CHANNEL_DMA))
...
static void sr_trace_dma_event(const SrRunnerEvent *event) {
    if (sr_trace_active() && sr_trace_channel_enabled(SR_TRACE_CHANNEL_DMA))
...
void sr_trace_dispmiss(uint32_t from_pc, uint32_t to_pc) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_DISPMISS)) return;
...
void sr_trace_garbage(uint32_t pc24, const char *name, int m, int x) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_GARBAGE)) return;
...
void sr_trace_wram(uint32_t offset, uint16_t old_value, uint16_t value,
                   int width) {
    if (!sr_trace_active()) return;
    if (sr_trace_channel_enabled(SR_TRACE_CHANNEL_STACK) && offset >= 0x100u && offset < 0x200u) {
        write_event("stack", ",\"off\":\"%05X\",\"old\":\"%04X\","
                    "\"val\":\"%04X\",\"w\":%d}\n",
                    offset, old_value, value, width);
    }
    if (sr_trace_channel_enabled(SR_TRACE_CHANNEL_WRAM) && offset >= s_wram_low &&
        offset <= s_wram_high) {
...
void sr_trace_hwread(uint16_t address, uint8_t value) {
    if (sr_trace_active() && sr_trace_channel_enabled(SR_TRACE_CHANNEL_HWREAD))
        write_event("hwread", ",\"reg\":\"%04X\",\"val\":\"%02X\"}\n",
                    address, value);
}

void sr_trace_ppumem(const char *memory, uint16_t address, uint16_t value) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_PPUMEM)) return;
...
void sr_trace_frame(const char *event) {
    if (!sr_trace_active() || !sr_trace_channel_enabled(SR_TRACE_CHANNEL_FRAME)) return;
...
static void observe_runner_event(void *user_data, SrRunnerHandle *runner,
    ...
    if (event->type == SR_EVENT_MEMORY_WRITE) {
        if (event->memory_region == SR_MEMORY_WRAM) {
            sr_trace_wram(event->address, (uint16_t)event->previous_value,
                          (uint16_t)event->value,
                          (int)event->width_bytes);
        } else if (sr_trace_active() && sr_trace_channel_enabled(SR_TRACE_CHANNEL_PPUMEM)) {
            ...
            if (name != NULL)
                sr_trace_ppumem(name, (uint16_t)event->address,
                                (uint16_t)event->value);
        }
    } else if (event->type == SR_EVENT_REGISTER_WRITE) {
        sr_trace_reg((uint16_t)event->address, (uint8_t)event->value);
...
void sr_trace_bind_runner(Snes *runner, int enabled) {
    ...
    if ((s_channels & (SR_TRACE_CHANNEL_WRAM | SR_TRACE_CHANNEL_STACK | SR_TRACE_CHANNEL_PPUMEM)) != 0) {
    ...
    if ((s_channels & (SR_TRACE_CHANNEL_REG | SR_TRACE_CHANNEL_HWREAD)) != 0) {
        subscription.event_mask = SR_EVENT_MASK_REGISTER_ACCESS;
        if (api->subscribe_events(s_runner, &subscription,
                                  &s_register_subscription) != SR_RESULT_OK)
            s_register_subscription = 0u;
    }
    if ((s_channels & SR_TRACE_CHANNEL_DMA) != 0) {
