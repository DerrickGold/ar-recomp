...
import (
	...
	"github.com/DerrickGold/snesrecomp-go/internal/quintet"
)

// Kept as a package-local seam for the existing extraction-oracle tests.
// Fonts and workshop art use the same native codec.
...
func (d *Decoder) assetScript() ([]assetEntry, error) {
	...
	for cursor < assetScriptLimit {
		...
		if entry.mode == 7 && entry.submode == 8 {
			return entries, nil
		}
	}
...
func (d *Decoder) dialogFont(entries []assetEntry) (IRObject, error) {
	...
	return IRObject{"status": "asset_script_source_verified", "classification": "regional_graphical_text_font", "source_file_offset": fileOffset(source), "source_pc24": cursorAddress(source),
		...
		"decoded_sha256": rawSHA256(decoded), "tile_count": len(decoded) / 16, "nonblank_tile_count": nonblank, "unique_tile_count": len(unique), "redistribution": "rom_derived_do_not_distribute"}, nil
}
