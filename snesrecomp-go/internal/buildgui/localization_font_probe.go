...
// A host adapter over the game's versioned, headless font command. The game
// executable comes only from build/detection state, never from imported packs.
func (app *application) probeLocalizationFonts(ctx context.Context, fonts []lk.FontCoverageSource, scalars []rune) (lk.FontCoverageProbeResult, error) {
	...
	return runLocalizationFontProbe(ctx, binary, builtin, fonts, scalars)
}
...
func runLocalizationFontProbe(ctx context.Context, binary, builtin string, fonts []lk.FontCoverageSource, scalars []rune) (lk.FontCoverageProbeResult, error) {
