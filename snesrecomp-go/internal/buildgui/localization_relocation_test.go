...
func TestLocalizationRelocatedBuilderGameWorkflow(t *testing.T) {
	...
	copyFile(builder, filepath.Join(utils, "tools", "snesbuild"), 0755)
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, filepath.Join(utils, "tools", "snesbuild"), "localization-extract", "--rom", rom, "--out", archive)
