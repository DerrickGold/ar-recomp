...
func SyncFuncsWithEntryFacts(cfgDir, outputPath string, entryFacts []analysis.EntryFact) (int, error) {
	...
	for _, path := range paths {
		...
		for _, entry := range cfg.Entries {
			...
			items = append(items, namedAddress{
				Name: entry.Name, Address: uint32(cfg.Bank)<<16 | uint32(entry.Start), AddressKnown: true,
			})
		}
		for _, declaration := range cfg.Names {
			...
			items = append(items, namedAddress{
				Name: declaration.Name, Address: declaration.Address & 0xffffff, AddressKnown: true,
			})
		}
		if cfg.Bank == 0 && cfg.AutoVectors {
			for _, name := range []string{"I_RESET", "I_NMI", "I_IRQ"} {
				if _, found := seen[name]; found {
					continue
				}
				seen[name] = struct{}{}
				items = append(items, namedAddress{Name: name})
			}
		}
	}
	for _, fact := range entryFacts {
		...
		seen[name] = struct{}{}
		items = append(items, namedAddress{
			Name: name, Address: fact.PC & 0xffffff, AddressKnown: true,
		})
	}
	sort.SliceStable(items, func(i, j int) bool {
		if items[i].AddressKnown != items[j].AddressKnown {
			return items[i].AddressKnown
		}
