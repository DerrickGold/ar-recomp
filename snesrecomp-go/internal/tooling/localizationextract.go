...
func RunLocalizationExtractCommand(args []string, cwd string, output io.Writer) error {
	...
	if flags.NArg() != 0 || *romPath == "" || *outPath == "" || (*format != "pack" && *format != "catalog" && *format != "runtime-routes" && *format != "compose-routes") {
		return fmt.Errorf("usage: snesbuild localization-extract --rom game.sfc --out new-file [--format pack|catalog|runtime-routes|compose-routes]")
	}
	...
	if *format == "pack" {
		...
		project, err := localizationkit.NewSourceProject(pack)
		if err != nil {
			return err
		}
...
func readLocalizationROM(path string) (*localizationkit.Decoder, error) {
	...
	return localizationkit.NewDecoder(data)
}
