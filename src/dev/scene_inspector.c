...
bool SceneInspector_SelectFiltered(int screen_x, int screen_y,
  ...
  if (s_sim_valid && s_sim.view != kSimView_None) {
    ...
    if (s_sim.world_navigation_state_valid) {
      ...
      Append(&panel,
             "COMP %s%s LABEL %u+%u PLAQUE %u+%u PALACE %u+%u "
             "FX L/C/B/H %u/%u/%u/%u\n",
             composition->valid ? "READY" : "FALLBACK",
             composition->empty_animation ? "/EMPTY" : "",
             (unsigned)composition->label.oam_first,
             (unsigned)composition->label.oam_count,
             (unsigned)composition->plaque.oam_first,
             (unsigned)composition->plaque.oam_count,
  ...
  if (s_sim_valid && s_sim.view != kSimView_None) {
    ...
    if (s_sim.world_navigation_state_valid) {
      ...
      Append(&report,
             ...
             "label=%u+%u bounds=%d,%d %ux%u "
             "plaque=%u+%u bounds=%d,%d %ux%u "
             "palace=%u+%u bounds=%d,%d %ux%u effects=%u/%u/%u/%u\n",
             composition->valid, composition->empty_animation,
             (unsigned)composition->label.oam_first,
             (unsigned)composition->label.oam_count,
             (int)composition->label.screen_x,
             (int)composition->label.screen_y,
             (unsigned)composition->label.width,
             (unsigned)composition->label.height,
             (unsigned)composition->plaque.oam_first,
             (unsigned)composition->plaque.oam_count,
             (int)composition->plaque.screen_x,
             (int)composition->plaque.screen_y,
             (unsigned)composition->plaque.width,
             (unsigned)composition->plaque.height,
             (unsigned)composition->palace.oam_first,
             (unsigned)composition->palace.oam_count,
