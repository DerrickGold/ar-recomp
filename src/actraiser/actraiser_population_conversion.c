#include "actraiser_population_conversion.h"
...
ActRaiserPopulationResult ActRaiserPopulation_Preview(CpuState *cpu,
  ...
  if (!ArRegionalSession_SetPopulationProfile(&candidate,candidate.revision,source))
    return kActRaiserPopulation_Unsafe;
...
ActRaiserPopulationResult ActRaiserPopulation_Commit(CpuState *cpu,
  ...
  const ActRaiserPopulationResult preview=ActRaiserPopulation_Preview(cpu,campaign,confirmed->source,&fresh);
  ...
  if(!ArRegionalSession_SetPopulationProfile(&candidate,confirmed->revision,confirmed->source))return kActRaiserPopulation_Stale;
